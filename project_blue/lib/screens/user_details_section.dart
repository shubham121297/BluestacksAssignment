...
class _UserDetailsSectionState extends State<UserDetailsSection> {
  ...
  void didChangeDependencies() {
    ...
    if(_isInIt){
           ...
           .then((value) {
             userData=UserDetails(imageUrl: value.imageUrl, name: value.name, played: value.played, rating: value.rating, userName: value.userName, winPercent: value.winPercent, won: value.won);
              setState(() {
                //_isLoading1=false;
                //print("hey");
         });
  ...
  Widget build(BuildContext context) {
    ...
    return _isLoading?Scaffold(body: Center(child:CircularProgressIndicator()),):Scaffold(
      appBar: AppBar(leading:(Icon(Icons.short_text,textDirection: TextDirection.rtl,color: Colors.black,size: 40,)),title: Container(margin: EdgeInsets.fromLTRB(90, 2, 100, 2),child: Text(widget.user.toString(),style: TextStyle(color: Colors.black),)),backgroundColor: Colors.white,elevation: 0,),
      body: Column(
        children: [Row(crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Column(
                ...
                children: [
                Padding(padding: EdgeInsets.fromLTRB(5,20,20,20),child: Text(userData.name,style: TextStyle(fontSize: 20,fontWeight: FontWeight.w500),)),
                Chip(backgroundColor: Colors.white,side: BorderSide(color: Colors.blueAccent),
                  label:Row(children: [
                    Text(userData.rating.toInt().toString(),style: TextStyle(fontSize: 20,fontWeight: FontWeight.w800,color: Colors.blueAccent),),
                    Padding(padding: EdgeInsets.fromLTRB(10,1,10,1),child: Text("Elo rating",style: TextStyle(fontSize: 12,fontWeight: FontWeight.w600),)),
                  ],)
                )
              ],)
            ]),
                            ...
                            child: ClipRRect(
                              ...
                              child: Container(
                                ...
                                child:Column(
                                  children: [
                                    SizedBox(height: 8),
                                    Text("${userData.winPercent.toInt().toString()}%",style: TextStyle(color: Colors.white,fontSize: 17),),
                                    SizedBox(height: 8),
        ...
        Align(alignment:Alignment.centerLeft,child: Padding(padding:EdgeInsets.fromLTRB(26, 10 , 20, 10),child: Text("Recommended for you",style: TextStyle(fontSize: 26,fontWeight: FontWeight.bold),))),
        Expanded(
          child: ListView.builder(
            itemCount: recData.length,
            itemBuilder: (ctx,index){
              return _isLoading?Center(child: CircularProgressIndicator(),):RecommendedFor(coverUrl: recData[index].coverUrl, gameName: recData[index].gameName, name: recData[index].name);
            }
            ),
        )
